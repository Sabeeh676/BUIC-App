...
class ManageMidtermPage extends StatefulWidget {
  const ManageMidtermPage({super.key});

  @override
  State<ManageMidtermPage> createState() => _ManageMidtermPageState();
}

class _ManageMidtermPageState extends State<ManageMidtermPage> {
  ...
  void _onSelectionChanged(String? courseId, List<String> classIds) {
    setState(() {
      ...
      if (_selectedCourseId != null && _selectedClassId != null) {
        _fetchMidtermDetails();
      }
    });
  }

  Future<void> _fetchMidtermDetails() async {
    setState(() => _isLoading = true);
    try {
      final midtermDoc = await FirebaseFirestore.instance
          .collection('courses')
      ...
      if (midtermDoc.exists) {
        _totalMarksController.text =
            midtermDoc.data()?['totalMarks']?.toString() ?? '';
      ...
      _showError('Failed to fetch midterm details.');
    } finally {
      setState(() => _isLoading = false);
    }
  }

  Future<void> _setMidtermDetails() async {
    ...
    try {
      ...
      _detailsSet = true;
      await _fetchStudentsAndMarks();
      _showSuccess('Midterm details saved.');
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Manage Midterm Marks')),
      body: Padding(
        ...
        children: [
          Row(
            children: [
              ...
              ElevatedButton(
                onPressed: _detailsSet ? null : _setMidtermDetails,
